...
class BASE_routing(metaclass=abc.ABCMeta):
    ...
    def drone_reception(self, src_drone, packet: Packet, current_ts):
        ...
        if isinstance(packet, HelloPacket):
            ...
            ack_packet = ACKPacket(self.drone, src_drone, None, None, None, None, self.simulator, packet, current_ts)
    ...
    def send_packets(self, cur_step):
        ...
        if cur_step % self.simulator.drone_retransmission_delta == 0:
            ...
            for pkd in self.drone.all_packets():
                ...
                # keep track of the packets that will be forwarded
                if best_neighbor != None and best_neighbor != self.drone:
                    to_be_sent_packets.append(pkd)

                # update the queue time for all the packets that still are waiting
                for packet in self.drone.all_packets():
                    if packet not in to_be_sent_packets and isinstance(packet, DataPacket):
                        packet.queue_time += 1

                if best_neighbor is not None:
                    
                    if isinstance(pkd, DataPacket): # set the forwarding time for the packet
                        pkd.time_of_data_forwarding = cur_step

                    self.unicast_message(pkd, self.drone, best_neighbor, cur_step)

                self.current_n_transmission += 1

            # reset the queue time for the packets that won't be forwarded
            for packet in self.drone.all_packets():
                if packet not in to_be_sent_packets and isinstance(packet, DataPacket):
                    packet.queue_time = 0
    ...
    def unicast_message(self, packet, src_drone, dst_drone, curr_step):
        """ send a message to my neigh drones"""
        self.drone.total_energy_consumption += 0.05
